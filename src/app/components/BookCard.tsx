...
const BookCard: React.FC<BookCardProps> = ({ book, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 hover:shadow-xl transition-transform duration-300">
      ...
      <Link href={`/book/${book.id}`} className="block cursor-pointer">
        <div className="relative h-64 w-full bg-gray-200 flex items-center justify-center hover:bg-gray-300 transition-colors duration-200">
          ...
          <div className="text-6xl text-gray-400 z-10">📚</div>
        </div>
      </Link>
      ...
      <div className="p-4">
        ...
        <div className="mt-2">
          {book.genre.slice(0, 2).map((g) => (
            <span key={g} className="inline-block bg-blue-100 text-blue-700 rounded-full px-2 py-1 text-xs font-semibold mr-2 mb-2">
              {g}
            </span>
          ))}
          {book.genre.length > 2 && (
